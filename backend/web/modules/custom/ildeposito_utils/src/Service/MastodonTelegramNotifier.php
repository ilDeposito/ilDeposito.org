...
final class MastodonTelegramNotifier {
  ...
  public function __construct(private readonly MastodonClient $mastodon, private readonly ClientInterface $httpClient, private readonly StateInterface $state, private readonly MetaWeeklyStatsReporter $weeklyStatsReporter) {}
  public function isConfigured(): bool { return getenv('ILDEPOSITO_ENV') === 'prod' && $this->mastodon->isConfigured() && getenv('TELEGRAM_NOTIFICHE_BOT_TOKEN') && getenv('TELEGRAM_NOTIFICHE_CHAT_ID'); }
  ...
  public function sync(): int {
    ...
    $accountId = (string) ($this->mastodon->verifyCredentials()['id'] ?? '');
    ...
    $sent=0; foreach($items as $item){if(!is_array($item)||!is_string($item['id']??NULL))continue;$this->telegram($this->format($item));$this->weeklyStatsReporter->recordMastodonEvent($item, $accountId);$this->state->set(self::STATE_LAST_ID,$item['id']);$sent++;} return $sent;
  }
  ...
  private function telegram(string $text): void { $this->httpClient->request('POST','https://api.telegram.org/bot'.getenv('TELEGRAM_NOTIFICHE_BOT_TOKEN').'/sendMessage',['form_params'=>['chat_id'=>getenv('TELEGRAM_NOTIFICHE_CHAT_ID'),'text'=>$text,'disable_web_page_preview'=>TRUE],'timeout'=>10]); }
