...
- `FbApiCheckCommand` (drush `ildeposito:fb-api-check`, alias `iufbapicheck`) — verifica con una chiamata di sola lettura che il System User possa ottenere il token della Pagina, cioè il prerequisito delle pubblicazioni. In caso di errore registra un errore `ildeposito_utils`, che in produzione attiva la notifica Telegram. Eseguirlo dal crontab ogni 3–6 ore.
- Replica Facebook → Telegram — il webhook pubblico `GET|POST /api/facebook/webhook` riceve soltanto le notifiche `feed` firmate dall'app Meta, le valida con `FB_APP_SECRET` e accoda il `post_id`. La coda viene eseguita subito dopo la risposta HTTP e da Drupal cron come fallback; `ildeposito:facebook-telegram-sync` (`iufbtgsync`) va eseguito ogni 5 minuti dal crontab host per recuperare una consegna webhook mancata. La sua prima esecuzione inizializza una baseline silenziosa (non pubblica contenuti preesistenti); poi considera solo i post successivi, con 10 minuti di sovrapposizione e deduplicazione DB. Un lease atomico di 10 minuti impedisce che webhook e cron pubblichino in parallelo lo stesso post; allo scadere recupera un job interrotto. Il post e' riletto dalla Graph API come Pagina: testo, link e foto mantengono il proprio tipo Telegram; ogni link verso ilDeposito riceve `utm_source=telegram`, `utm_medium=social`, `utm_campaign=facebook_sync` e `utm_content=facebook_post` per Umami. Le foto oltre 10 MB sono ridotte in JPEG tramite GD prima dell'upload. La replica Instagram accetta solo foto native singole: se Meta rifiuta il contenitore per aspect ratio fuori range (errore 36003/2207009, il range API e' 4:5–1.91:1) il publisher riprova una sola volta con un derivato con letterbox (bande nere, mai crop) salvato in `public://instagram` ed esposto via `PUBLIC_BACKEND_URL`; se anche quello fallisce la foto e' marcata `ignored` su Instagram senza bloccare Telegram e Mastodon. Un link a `/eventi/` di ilDeposito diventa volutamente un messaggio-link per sfruttare la preview OpenGraph del sito. L'ordine di replica e' Instagram, Mastodon e per ultimo Telegram (di proposito: la trattenuta mattutina non blocca gli altri due). Solo il primo post automatico del giorno (link `/eventi/`) esce su Telegram non prima delle 8:30 Europe/Rome (fuso esplicito nel codice, slot giornaliero in State consumato alla prima trattenuta o al primo invio; dopo le 8:30 esce subito anche se nessun giro coda e' passato nella finestra): i successivi automatici e tutti i manuali restano istantanei. Configurare solo in produzione `FB_WEBHOOK_VERIFY_TOKEN`, `TELEGRAM_CANALE_BOT_TOKEN` e `TELEGRAM_CANALE_CHAT_ID` (separati dagli alert tecnici); rendere il bot amministratore del canale. In Meta configurare il callback HTTPS, sottoscrivere il campo `feed` e associare l'app alla Pagina. Dopo il deploy eseguire `drush updb -y && drush cr`.
