...
export const GET: APIRoute = async ({ request }) => {
  console.log(`${tag} GET da ${request.headers.get('user-agent')?.slice(0, 40)} origin=${request.headers.get('origin')} referer=${request.headers.get('referer')?.slice(0, 60)}`);
  const masterKey = (import.meta.env.ALTCHA_HMAC_KEY as string) || process.env.ALTCHA_HMAC_KEY || '';

  if (!masterKey) {
    console.error(`${tag} ALTCHA_HMAC_KEY non configurata`);
  ...
  try {
    ...
    console.error(`${tag} Errore creazione challenge:`, err);
